...
namespace MultiTool.Core.Validation;
...
public sealed class SettingsValidator
{
    ...
    public ValidationResult ValidateClickSettings(ClickSettings settings)
    {
        ...
        if (settings.RandomTimingVarianceMilliseconds < 0)
        {
            issues.Add(new ValidationIssue(nameof(settings.RandomTimingVarianceMilliseconds), "Random timing variance cannot be negative."));
        }
