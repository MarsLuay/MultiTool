...
public partial class MainWindowViewModel
{
    ...
    public ObservableCollection<Ipv4SocketEntry> Ipv4SocketEntries { get; } = [];
    ...
    public bool HasIpv4SocketEntries => Ipv4SocketEntries.Count > 0;
    ...
    public string Ipv4SocketSummary => BuildIpv4SocketSummary();
    ...
    [ObservableProperty]
    private string ipv4SocketStatusMessage = AppLanguageStrings.GetForCurrentLanguage(AppLanguageKeys.ToolsStatusIpv4SocketInitial);
    ...
    private DateTimeOffset lastIpv4SocketSnapshotCapturedAt;
    private int lastIpv4SocketTcpConnectionCount;
    ...
    private bool CanCaptureIpv4SocketSnapshot => !IsToolBusy;

    [RelayCommand(CanExecute = nameof(CanCaptureIpv4SocketSnapshot))]
    private async Task CaptureIpv4SocketSnapshotAsync()
    {
        IsToolBusy = true;
        Ipv4SocketStatusMessage = L(AppLanguageKeys.ToolsStatusIpv4SocketRefreshing);
        AddToolLog(Ipv4SocketStatusMessage);

        try
        {
            await TryCaptureIpv4SocketSnapshotAsync(applyToViewer: true).ConfigureAwait(true);
            AddToolLog(Ipv4SocketStatusMessage);
        }
        finally
        {
    ...
    private bool CanCopyIpv4SocketSnapshot => !IsToolBusy && Ipv4SocketEntries.Count > 0;

    [RelayCommand(CanExecute = nameof(CanCopyIpv4SocketSnapshot))]
    private void CopyIpv4SocketSnapshot()
    {
        try
        {
            clipboardTextService.SetText(BuildIpv4SocketClipboardText());
            Ipv4SocketStatusMessage = L(AppLanguageKeys.ToolsStatusIpv4SocketCopiedClipboard);
            AddToolLog(Ipv4SocketStatusMessage);
        }
        catch (Exception ex)
        {
            Ipv4SocketStatusMessage = F(AppLanguageKeys.ToolsStatusIpv4SocketCopyFailedFormat, ex.Message);
            AddToolLog(Ipv4SocketStatusMessage);
        }
    }
    ...
    public void CapturePinWindowHotkey(Key key)
    {
    ...
    private async Task ApplyTelemetryReductionAsync()
    {
        ...
        try
        {
            var beforeSnapshot = await TryCaptureIpv4SocketSnapshotAsync(applyToViewer: false).ConfigureAwait(true);
            ...
            var afterSnapshot = await TryCaptureIpv4SocketSnapshotAsync(applyToViewer: true).ConfigureAwait(true);
            var telemetryStatusMessage = GetTelemetryStatusMessageOrFallback(result.Message);
            var ipv4SourcesMessage = beforeSnapshot is not null && afterSnapshot is not null
                ? BuildTelemetryIpv4SourcesMessage(beforeSnapshot, afterSnapshot)
                : L(AppLanguageKeys.ToolsStatusTelemetryIpv4SourcesUnavailable);

            TelemetryToolStatusMessage = $"{telemetryStatusMessage} {ipv4SourcesMessage}";
            AddToolLog(TelemetryToolStatusMessage);
        }
        catch (Exception ex)
        {
    ...
    private string GetTelemetryStatusMessageOrFallback(string fallbackMessage)
    {
        try
        {
            return windowsTelemetryService.GetStatus().Message;
        }
        catch
        {
    ...
    private async Task ScanDriverUpdatesCoreAsync(bool addLogEntry, bool manageBusyState)
    {
        ...
        DriverUpdateStatusMessage = isUsingCachedHardwareCheck
            ...
            : L(AppLanguageKeys.ToolsStatusDriverScanStarting);
        if (addLogEntry)
        {
            AddToolLog(DriverUpdateStatusMessage);
        }
    ...
    private void RefreshToolCommandStates()
    {
        ...
        CaptureIpv4SocketSnapshotCommand.NotifyCanExecuteChanged();
        CopyIpv4SocketSnapshotCommand.NotifyCanExecuteChanged();
    ...
    private string BuildIpv4SocketSummary()
    {
        if (Ipv4SocketEntries.Count == 0)
        {
        ...
        return F(
            ...
            lastIpv4SocketSnapshotCapturedAt.ToLocalTime().ToString("HH:mm:ss"));
    }

    private async Task<Ipv4SocketSnapshotResult?> TryCaptureIpv4SocketSnapshotAsync(bool applyToViewer)
    {
        try
        {
            var result = await ipv4SocketSnapshotService.CaptureAsync().ConfigureAwait(true);
            if (applyToViewer)
            {
                ApplyIpv4SocketSnapshot(result);
                Ipv4SocketStatusMessage = BuildIpv4SocketStatusMessage(result);
            }
        ...
        catch (Exception ex)
        {
            if (applyToViewer)
            {
                Ipv4SocketStatusMessage = F(AppLanguageKeys.ToolsStatusIpv4SocketFailedFormat, ex.Message);
            }
    ...
    {
        var beforeSourceCount = CountIpv4RemoteSources(beforeSnapshot);
        var afterSourceCount = CountIpv4RemoteSources(afterSnapshot);

        return beforeSourceCount == afterSourceCount
            ? F(
                AppLanguageKeys.ToolsStatusTelemetryIpv4SourcesUnchangedFormat,
                beforeSourceCount,
                beforeSourceCount == 1 ? string.Empty : "s")
            : F(
                AppLanguageKeys.ToolsStatusTelemetryIpv4SourcesChangedFormat,
                beforeSourceCount,
                beforeSourceCount == 1 ? string.Empty : "s",
                afterSourceCount,
                afterSourceCount == 1 ? string.Empty : "s");
    }

    private static int CountIpv4RemoteSources(Ipv4SocketSnapshotResult snapshot)
    {
        var sources = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

        foreach (var entry in snapshot.Entries)
        {
            var source = TryExtractIpv4RemoteSource(entry.RemoteEndpoint);
    ...
    {
        if (string.IsNullOrWhiteSpace(remoteEndpoint) || string.Equals(remoteEndpoint, "*:*", StringComparison.Ordinal))
        {
            return null;
        }

        var separatorIndex = remoteEndpoint.LastIndexOf(':');
        if (separatorIndex <= 0)
        {
            return null;
        }
    ...
    private void ApplyIpv4SocketSnapshot(Ipv4SocketSnapshotResult result)
    {
        Ipv4SocketEntries.Clear();
        foreach (var entry in result.Entries)
        {
            Ipv4SocketEntries.Add(entry);
        }

        lastIpv4SocketSnapshotCapturedAt = result.CapturedAt;
        lastIpv4SocketTcpConnectionCount = result.TcpConnectionCount;
        lastIpv4SocketTcpListenerCount = result.TcpListenerCount;
    ...
    private string BuildIpv4SocketClipboardText()
    {
        var builder = new StringBuilder();
        builder.AppendLine(L(AppLanguageKeys.ToolsIpv4SocketClipboardTitle));
        builder.AppendLine(F(
            AppLanguageKeys.ToolsIpv4SocketClipboardCapturedFormat,
            lastIpv4SocketSnapshotCapturedAt.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss")));
        builder.AppendLine(BuildIpv4SocketSummary());
        builder.AppendLine();
