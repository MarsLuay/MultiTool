...
public sealed class WindowsHardwareInventoryService : IHardwareInventoryService
{
    ...
    private static HardwareInventoryReport BuildReport()
    {
        ...
        try
        {
            driverHardwareInventory = WindowsDriverUpdateService.ReadHardwareInventory();
        }
        catch (Exception ex)
        {
            warnings.Add($"Driver hardware inventory skipped: {ex.Message}");
            driverHardwareInventory = [];
        }
