...
namespace MultiTool.Infrastructure.Windows.Screenshot;
...
public sealed class WindowsScreenshotCaptureService : IScreenshotCaptureService, IDisposable
{
    ...
    public async Task StartVideoCaptureAsync(string outputDirectory, string fileNamePrefix, ScreenRectangle? area = null, CancellationToken cancellationToken = default)
    {
        ...
        var shouldRaiseStartEvent = false;
        ...
        try
        {
            ...
            lock (syncRoot)
            {
                ...
                currentVideoArea = area;
            }

            shouldRaiseStartEvent = true;
        }
        ...
        if (shouldRaiseStartEvent)
        {
            VideoCaptureStateChanged?.Invoke(this, new VideoCaptureStateChangedEventArgs(isRecording: true, area));
        }
    }
    ...
    public async Task<string?> StopVideoCaptureAsync(CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        await videoLock.WaitAsync(cancellationToken);
        var shouldRaiseStopEvent = false;
        ScreenRectangle? stoppedArea = null;
        ...
        try
        {
            ...
            lock (syncRoot)
            {
                ...
                stoppedArea = currentVideoArea;
                currentVideoArea = null;
            }

            savedPath = currentVideoPath;
            currentVideoPath = null;
            shouldRaiseStopEvent = true;
        ...
        if (shouldRaiseStopEvent)
        {
            VideoCaptureStateChanged?.Invoke(this, new VideoCaptureStateChangedEventArgs(isRecording: false, stoppedArea));
        }
    ...
    public void Dispose()
    {
        ...
        lock (syncRoot)
        {
            ...
            videoCaptureProcess = null;
            currentVideoArea = null;
            shouldRaiseStopEvent = process is not null;
        }
