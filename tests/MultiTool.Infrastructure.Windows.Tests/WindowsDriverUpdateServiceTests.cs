...
public sealed class WindowsDriverUpdateServiceTests
{
    ...
    [Fact]
    public async Task ScanAsync_ShouldReuseProvidedHardwareInventoryWithoutCallingReader()
    {
        IReadOnlyList<DriverHardwareInfo> cachedHardware =
        [
            new DriverHardwareInfo("GPU", "NVIDIA", "NVIDIA", "1.0.0", "Display", "PCI\\VEN_10DE"),
        ];

        var service = new WindowsDriverUpdateService(
            () => throw new InvalidOperationException("Hardware reader should not run"),
            () =>
            [
                new DriverUpdateCandidate("update-1", "NVIDIA Display Driver", "RTX 4080", "NVIDIA", "Display", "2026-03-01", "Recommended display update.", false, false),
            ],
            _ => []);

        var result = await service.ScanAsync(cachedHardware);

        result.Hardware.Should().BeEquivalentTo(cachedHardware);
        result.Updates.Should().ContainSingle();
        result.Warnings.Should().BeEmpty();
    }
