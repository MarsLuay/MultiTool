...
public sealed class AutoClickerController : IAutoClickerController
{
    ...
    public AutoClickerController(IMouseInputService mouseInputService, ICursorService cursorService)
    {
    ...
    private async Task RunLoopAsync(ClickSettings settings, CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    await Task.Delay(nextDue, cancellationToken).ConfigureAwait(false);
                }
